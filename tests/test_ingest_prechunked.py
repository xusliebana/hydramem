...
def pipeline_accept():
    ...
    return IngestionPipeline(store=store, embedder=embedder, verifier=_accept_all_verifier())
...
def pipeline_reject():
    ...
    embedder = MagicMock()
    embedder.embed_batch.side_effect = lambda texts: [[0.1] * 384 for _ in texts]
    return IngestionPipeline(store=store, embedder=embedder, verifier=_reject_all_verifier())
...
class TestIngestPrechunked:
        ...
        payload = [
            {
                ...
                "relations": [
                    {"from": "HydraMem", "to": "LanceDB", "type": "USES", "confidence": 0.9},
                ],
            },
            {
        ...
        payload = [
            {
                ...
                "relations": [
                    {"from": "X", "to": "Y_NOT_DECLARED", "type": "REL", "confidence": 0.9},
                ],
            }
        ]
        result = pipeline_accept.ingest_prechunked(
    ...
    def test_chunk_limit_truncates(self, pipeline_accept):
        ...
        payload = [
            {"text": f"chunk {i}", "entities": [{"name": f"E{i}", "type": "x"}]} for i in range(10)
        ]
        result = pipeline_accept.ingest_prechunked(
            source="big.md",
            chunks=payload,
    ...
    def test_invalid_payload_raises(self, pipeline_accept):
        with pytest.raises(TypeError):
            pipeline_accept.ingest_prechunked(
                source="x",
                chunks="not a list",
                project="t",  # type: ignore[arg-type]
            )
...
class TestSubmitSessionExtraction:
    def test_basic_session_dump(self, pipeline_accept):
        result = pipeline_accept.submit_session_extraction(
            ...
            relations=[
                {"from": "Alpha", "to": "Beta", "type": "REL", "confidence": 0.8},
            ],
        )
        assert result["entities_added"] == 2
        assert result["relations_proposed"] == 1
    ...
    def test_unknown_endpoint_rejected(self, pipeline_accept):
        result = pipeline_accept.submit_session_extraction(
            ...
            relations=[
                {"from": "Alpha", "to": "Missing", "type": "REL", "confidence": 0.8},
            ],
        )
        assert result["relations_accepted"] == 0
        assert result["relations_rejected"] == 1
