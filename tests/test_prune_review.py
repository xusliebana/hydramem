...
class TestPruneReviewStore:
    def test_add_dedup_and_stats(self, tmp_path):
        ...
        assert (
            s.add_candidate(project="p", from_id="a", to_id="b", spuriousness=0.7, features=feats)
            is True
        )
        ...
        assert (
            s.add_candidate(project="p", from_id="c", to_id="d", spuriousness=0.6, features=feats)
            is True
        )
        st = s.stats("p")
    ...
    def test_label_builds_golden_dataset(self, tmp_path):
        s = _store(tmp_path)
        s.add_candidate(
            project="p", from_id="a", to_id="b", spuriousness=0.7, features={"heuristic": 0.7}
        )
        s.add_candidate(
            project="p", from_id="c", to_id="d", spuriousness=0.6, features={"heuristic": 0.6}
        )
...
class TestTrainPruner:
    ...
    def test_refuses_single_class(self, tmp_path):
        ...
        for i in range(25):
            s.add_candidate(
                project="p", from_id=f"u{i}", to_id=f"v{i}", features={"heuristic": 0.9}
            )
...
class TestLearnedScorerAndCLI:
    ...
    def test_auto_train_when_enabled_and_enough_labels(self, tmp_path, monkeypatch):
        ...
        for i in range(24):
            ...
            review.label(review.pending("p", limit=1)[0]["id"], "prune" if i % 2 == 0 else "keep")
        cfg = load_config({"night_gardener": {"review": {"enabled": True, "auto_train": True}}})
        g = NightGardener(
            store=_small_graph_store(),
            inferrer=MagicMock(),
            pipeline=MagicMock(),
