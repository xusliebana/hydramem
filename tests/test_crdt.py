...
def test_merge_preserves_distinct_sessions():
    local = [{"session_id": "s1", "project": "demo", "entries": [_entry("a", "2026-05-07T10:00:00+00:00")]}]
    remote = [{"session_id": "s2", "project": "demo", "entries": [_entry("b", "2026-05-07T11:00:00+00:00")]}]
    merged = merge_sessions(local, remote)
...
def test_merge_session_files_writes_output(tmp_path):
    ...
    local_path.write_text(json.dumps([
        {"session_id": "s1", "project": "demo", "entries": [_entry("x", "2026-05-07T10:00:00+00:00")]}
    ]))
    remote_path.write_text(json.dumps([
        {"session_id": "s1", "project": "demo", "entries": [_entry("y", "2026-05-07T11:00:00+00:00")]}
    ]))
